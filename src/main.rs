...
#[cfg(target_os = "android")]
fn default_android_folder_candidates() -> Vec<PathBuf> {
    vec![
        PathBuf::from("/storage/emulated/0/stepmania"),
        PathBuf::from("/storage/emulated/0/StepMania"),
        PathBuf::from("/storage/emulated/0/Download"),
        PathBuf::from("/storage/emulated/0/Music"),
        PathBuf::from("/storage/emulated/0"),
        PathBuf::from("/sdcard/stepmania"),
        PathBuf::from("/sdcard/Download"),
        PathBuf::from("/sdcard/Music"),
        PathBuf::from("/sdcard"),
    ]
}
...
#[cfg(target_os = "android")]
fn save_song_folder(path: &Path, folder: &Path) {
    if let Some(parent) = path.parent() {
        let _ = fs::create_dir_all(parent);
    }
    let _ = fs::write(path, folder.to_string_lossy().to_string());
}
...
#[cfg(target_os = "android")]
async fn choose_song_folder_first_run(
    save_path: &Path,
    ui_font: Option<&Font>,
) -> Option<PathBuf> {
    if let Some(saved) = load_saved_song_folder(save_path) {
        return Some(saved);
    }

    let mut current = default_android_folder_candidates()
        .into_iter()
        .find(|p| p.is_dir())
    ...
    loop {
        let dirs = list_child_dirs(&current);
        selected = selected.min(dirs.len().saturating_sub(1));

        if is_key_pressed(KeyCode::Up) {
            selected = selected.saturating_sub(1);
        }
        if is_key_pressed(KeyCode::Down) && selected + 1 < dirs.len() {
            selected += 1;
        }
        ...
        if is_key_pressed(KeyCode::Enter) {
            if let Some(folder) = dirs.get(selected) {
        ...
        if is_mouse_button_pressed(MouseButton::Left) {
            let choose_rect = Rect::new(sw - 238.0, 88.0, 196.0, 54.0);
            let up_rect = Rect::new(36.0, 88.0, 128.0, 54.0);
            if choose_rect.contains(vec2(mx, my)) {
                if folder_contains_charts(&current) {
                    save_song_folder(save_path, &current);
                    return Some(current);
                }
                error_message = "No .sm/.ssc/.dwi charts found in this folder.".to_string();
            } else if up_rect.contains(vec2(mx, my)) {
                if let Some(parent) = current.parent() {
                    current = parent.to_path_buf();
                    selected = 0;
                    error_message.clear();
                }
            } else if my >= top && my <= sh - 34.0 {
                let row = ((my - top) / row_h).floor() as usize;
                if let Some(folder) = dirs.get(row) {
                    current = folder.clone();
                    selected = 0;
                    error_message.clear();
                }
        ...
        draw_text_ui(ui_font, &current.to_string_lossy(), pad, 80.0, 22.0, LIGHTGRAY);
        draw_button(ui_font, 36.0, 88.0, 128.0, 54.0, "Up", Color::from_rgba(42, 52, 72, 255));
        draw_button(
            ui_font,
            sw - 238.0,
            88.0,
        ...
        if !error_message.is_empty() {
            draw_text_ui(ui_font, &error_message, pad, 126.0, 22.0, Color::from_rgba(255, 205, 90, 255));
        }

        if dirs.is_empty() {
            draw_text_ui(ui_font, "No folders here.", pad, top + 42.0, 30.0, GRAY);
        } else {
            let visible = ((sh - top - 34.0) / row_h).floor().max(1.0) as usize;
            for (i, dir) in dirs.iter().take(visible).enumerate() {
                ...
                draw_rectangle(
                    ...
                    row_h - 8.0,
                    if selected_row {
                        Color::from_rgba(42, 64, 92, 255)
                    } else {
                        Color::from_rgba(22, 30, 46, 220)
                    },
                );
                let name = dir.file_name().and_then(|s| s.to_str()).unwrap_or("/");
                draw_text_ui(ui_font, name, pad + 16.0, y + 36.0, 28.0, WHITE);
            }
        }
        next_frame().await;
    }
...
async fn choose_song_folder_first_run(
    save_path: &Path,
    _ui_font: Option<&Font>,
) -> Option<PathBuf> {
    load_saved_song_folder(save_path)
}

#[cfg(target_os = "android")]
fn list_child_dirs(folder: &Path) -> Vec<PathBuf> {
...
async fn song_select(
    ...
    cover_textures: &mut Vec<Option<Option<Texture2D>>>,
    ...
    if cover_textures.len() != entries.len() {
        cover_textures.clear();
        cover_textures.resize(entries.len(), None);
    }
    ...
    loop {
        ...
        for &ri in request_queue.iter().take(4) {
            cover_loader.request(ri, entries[ri].jacket_path.as_deref(), cover_textures);
        }
        cover_loader.poll_upload(cover_textures).await;
...
async fn main() {
    ...
    let mut cover_textures_cache: Vec<Option<Option<Texture2D>>> = vec![None; entries.len()];
    loop {
...
#[cfg(target_os = "android")]
fn draw_button(font: Option<&Font>, x: f32, y: f32, w: f32, h: f32, label: &str, color: Color) {
    draw_rectangle(x, y, w, h, color);
    draw_rectangle_lines(x, y, w, h, 2.0, Color::from_rgba(255, 255, 255, 70));
    let size = 22.0;
    let text_dims = measure_text(label, font, size as u16, 1.0);
    draw_text_ui(
        font,
        label,
        x + (w - text_dims.width) * 0.5,
        y + (h + text_dims.height) * 0.5 - 4.0,
        size,
        WHITE,
    );
}
