...
pub struct CoverTextureLoader {
    pending: Vec<(usize, mpsc::Receiver<Option<PathBuf>>)>,
}
...
impl CoverTextureLoader {
    ...
    pub fn request(&mut self, idx: usize, src: Option<&Path>, textures: &[Option<Option<Texture2D>>]) {
        if idx >= textures.len() || textures[idx].is_some() {
            return;
        }
        if self.pending.iter().any(|(i, _)| *i == idx) {
            return;
        }
        ...
        self.pending.push((idx, rx));
    }

    pub async fn poll_upload(&mut self, textures: &mut [Option<Option<Texture2D>>]) {
        let mut still_pending: Vec<(usize, mpsc::Receiver<Option<PathBuf>>)> = Vec::new();
        for (idx, rx) in self.pending.drain(..) {
            match rx.try_recv() {
                Ok(Some(path)) => {
                ...
                Err(mpsc::TryRecvError::Empty) => still_pending.push((idx, rx)),
