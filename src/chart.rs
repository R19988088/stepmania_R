...
const ROWS_PER_BEAT: f32 = 48.0;

fn beat_to_row(beat: f32) -> i32 {
    (beat * ROWS_PER_BEAT).round() as i32
}

fn row_to_beat(row: i32) -> f32 {
    row as f32 / ROWS_PER_BEAT
}
        ...
        self.stops
            .sort_by(|a, b| {
                a.beat
                    ...
                    .then_with(|| b.is_delay.cmp(&a.is_delay))
            });
...
pub fn beat_to_seconds(beat: f32, segments: &[BpmSegment], stops: &[StopSegment]) -> f32 {
    ...
    while i < segments.len() {
        ...
        if end > start {
            ...
            time += (end as f64 - start as f64) * (60.0_f64 / bpm as f64);
        }
    ...
    for stop in stops {
        // 与 StepMania 对齐：同拍 stop/delay 不计入该拍音符时间（只影响后续拍）。
        if stop.beat < beat {
            time += stop.seconds.max(0.0) as f64;
        } else {
            break;
        }
    }
...
pub fn seconds_to_beat(seconds: f32, segments: &[BpmSegment], stops: &[StopSegment]) -> f32 {
    ...
    let mut curr_row = beat_to_row(beat);
    let mut bpm_idx = 0usize;
    let mut stop_idx = 0usize;

    while bpm_idx + 1 < segments.len() && beat_to_row(segments[bpm_idx + 1].beat) <= curr_row {
        bpm_idx += 1;
    }
    while stop_idx < stops.len() && beat_to_row(stops[stop_idx].beat) < curr_row {
        stop_idx += 1;
    }
    ...
    loop {
        ...
        let sec_per_row = sec_per_beat / ROWS_PER_BEAT;

        let next_bpm_row = segments
            .iter()
            .skip(bpm_idx + 1)
            .map(|s| beat_to_row(s.beat))
            .find(|r| *r > curr_row)
            .unwrap_or(i32::MAX);
        let next_stop_row = stops
            .iter()
            .skip(stop_idx)
            .map(|s| beat_to_row(s.beat))
            .find(|r| *r > curr_row)
            .unwrap_or(i32::MAX);
        let next_event_row = next_bpm_row.min(next_stop_row);

        if next_event_row == i32::MAX {
            let remain = (target - time).max(0.0);
            return row_to_beat(curr_row) + remain / sec_per_beat;
        }

        let row_span = (next_event_row - curr_row).max(0);
        let move_secs = row_span as f32 * sec_per_row;
        if target < time + move_secs {
            let progressed_rows = ((target - time) / sec_per_row).max(0.0);
            return row_to_beat(curr_row) + progressed_rows / ROWS_PER_BEAT;
        }

        time += move_secs;
        curr_row = next_event_row;
        beat = row_to_beat(curr_row);
    }
}
